# How the analysis works
  ...
  Layer 2 — Stream grouping: Packets are grouped by their TCP/UDP stream number (a 5-tuple: src IP, dst IP, src port, dst port, protocol). Hundreds of packets belonging to the same TCP
  connection become one stream entry.

  Layer 3 — Fingerprint requirement: The printout() function in ja4.py discards any stream that doesn't have a JA4 fingerprint. A JA4 fingerprint is only generated when the stream contains
  a TLS ClientHello (handshake type 1). So:
